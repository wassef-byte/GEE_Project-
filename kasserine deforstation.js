...
var l4 = ee.ImageCollection("LANDSAT/LT04/C02/T1_L2"),
    ...
    table = ee.FeatureCollection("FAO/GAUL_SIMPLIFIED_500m/2015/level1"),
    table2 = ee.FeatureCollection("FAO/GAUL_SIMPLIFIED_500m/2015/level2");
//FAO GAUL 500m Simplified: Global Administrative Unit Layers 2015, Second-Level Administrative Units
var roi = table2.filter(ee.Filter.or(
    ee.Filter.eq('ADM2_NAME', 'Thala'),
    ee.Filter.eq('ADM2_NAME', 'Foussana'),
    ee.Filter.eq('ADM2_NAME', 'Hidra')
  ));
  ...
  var forestCol = ee.ImageCollection(yearList.map(function(year){
    ...
    // Forest
    var forest = vi.gt(0.12362562873010992).selfMask().rename('forest').toUint16();
